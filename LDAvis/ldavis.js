LDAvis = function(to_select, json_file) {
	...
	if (xdiff > ydiff) {
        ...
        points.append("text")
            .attr("class", "txt")   // important: keep class=txt because LDAvis resizes these
            .attr("x", d => xScale(+d.x))
            .attr("y", d => yScale(+d.y) - 6)   // <-- move UP
            .attr("stroke", "black")
            .attr("opacity", 1)
        ...
        points.append("text")
            .attr("class", "txtfreq")  // different class so it is NOT resized by term_on/term_off
            .attr("x", d => xScale(+d.x))
            .attr("y", d => yScale(+d.y) + 14)   // <-- move DOWN (increase this number for more spacing)
            .attr("stroke", "black")
            .attr("opacity", 1)
            .style("text-anchor", "middle")
            .style("font-size", "15px")
            .style("font-weight", 400)
            .text(d => d3.format(".1f")(d.Freq) + "%");
